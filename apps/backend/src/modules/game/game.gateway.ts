...
export class GameGateway {
  ...
  async handleStartGame(@MessageBody() data: { roomId: string }, @ConnectedSocket() client: AuthenticatedSocket): Promise<void> {
    try {
      ...
      for (const player of gameState.players) {
        ...
        const playerSocket = playerSockets.find((s) => (s as unknown as AuthenticatedSocket).data.user?.userId === player.userId);
  ...
  async handleConfirmRole(@MessageBody() data: { roomId: string }, @ConnectedSocket() client: AuthenticatedSocket): Promise<void> {
    try {
      const userId = client.data.user?.userId;
      if (!userId) {
        throw new WsException("Unauthorized");
  ...
  async handleSubmitClue(@MessageBody() dto: SubmitClueDto, @ConnectedSocket() client: AuthenticatedSocket): Promise<void> {
    try {
      const userId = client.data.user?.userId;
      if (!userId) {
        throw new WsException("Unauthorized");
  ...
  async handleSubmitVote(@MessageBody() dto: SubmitVoteDto, @ConnectedSocket() client: AuthenticatedSocket): Promise<void> {
    try {
      const userId = client.data.user?.userId;
      if (!userId) {
        throw new WsException("Unauthorized");
      ...
      if (gameState.phase === GamePhase.RESULTS || gameState.phase === GamePhase.VICTORY || gameState.phase === GamePhase.TIE_BREAKER) {
        this.server.to(dto.roomId).emit("game:phaseChanged", {
          phase: gameState.phase,
          gameState,
        });
  ...
  async handleResolveTie(@MessageBody() data: { roomId: string }, @ConnectedSocket() client: AuthenticatedSocket): Promise<void> {
    try {
  ...
  async handleContinueNextRound(@MessageBody() data: { roomId: string }, @ConnectedSocket() client: AuthenticatedSocket): Promise<void> {
    try {
  ...
  async handleEndGame(@MessageBody() data: { roomId: string }, @ConnectedSocket() client: AuthenticatedSocket): Promise<void> {
    try {
  ...
  async handleGetGameState(@MessageBody() data: { roomId: string }, @ConnectedSocket() client: AuthenticatedSocket): Promise<void> {
    try {
      const gameState = await this.gameService.getGameState(data.roomId);
