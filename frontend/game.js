...
function displayRoomsList(rooms) {
  ...
  if (rooms.length === 0) {
    roomsList.innerHTML =
      '<p class="empty-state">No hay salas disponibles. Crea una nueva sala.</p>';
    return;
  }
...
function displayCluesPhase(gameState) {
  ...
  if (isMyTurn) {
    if (playerRoleElement) {
      playerRoleElement.textContent = currentTurnPlayer.isImpostor
        ? 'Rol: Impostor'
        : 'Rol: Ciudadano';
    ...
    if (instructionsElement) {
      if (currentTurnPlayer.isImpostor) {
        instructionsElement.textContent =
          'Eres el impostor. Da una pista basada en lo que has escuchado.';
      } else {
...
function updateVotingUI(gameState) {
  ...
  if (isMyTurn) {
    ...
    votingContainer.innerHTML =
      '<p class="empty-state">Esperando a que otros jugadores voten...</p>';
  }
